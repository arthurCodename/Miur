...
const checkoutSchema = z
  .object({
    fullName: z
      ...
      .regex(/^[\p{L}][\p{L} '\-\.]+$/u, "Imię i nazwisko może zawierać tylko litery, spacje i myślniki"),
    email: z.string().min(1, "Podaj e-mail").email("Nieprawidłowy adres e-mail"),
    phone: z
      .string()
      .min(1, "Podaj numer telefonu")
      .regex(PL_PHONE_REGEX, "Nieprawidłowy numer telefonu (9 cyfr, opcjonalnie z prefiksem +48)"),
    ...
    acceptPrivacy: z
      .boolean()
      .refine((v) => v === true, { message: "Potwierdzenie zapoznania się z polityką prywatności jest wymagane" }),
  ...
  .superRefine((data, ctx) => {
    ...
    if (data.deliveryMethod === "courier") {
      ...
      if (!data.city || data.city.length < 2) {
        ctx.addIssue({ path: ["city"], code: "custom", message: "Podaj miasto" });
      }
...
export default function CheckoutPage() {
  ...
  const [selectedPaczkomat, setSelectedPaczkomat] = useState<{ name: string; address: string } | null>(
    null,
  );
      ...
      <div className="bg-white">
        ...
        <form className="mt-10 flex flex-col gap-8" onSubmit={handleSubmit(onSubmit)} noValidate>
          ...
          <fieldset className="flex flex-col gap-4">
            ...
            <FormField label="Imię i nazwisko" htmlFor="fullName" error={errors.fullName?.message}>
              <input
                id="fullName"
                type="text"
                autoComplete="name"
            ...
            {deliveryMethod === "paczkomat" ? (
              <div className="flex flex-col gap-3">
                ...
                {selectedPaczkomat ? (
                  <p className="rounded-md border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm leading-relaxed text-zinc-800">
                    <span className="font-semibold text-zinc-900">Wybrany Paczkomat: </span>
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="grid gap-4 sm:grid-cols-[1fr_140px]">
                  <FormField label="Miasto" htmlFor="city" error={errors.city?.message}>
                    <input
                      id="city"
                      type="text"
                  ...
                  <FormField label="Kod pocztowy" htmlFor="postalCode" error={errors.postalCode?.message}>
                    <input
                      id="postalCode"
                      type="text"
                      placeholder="00-001"
          ...
          <fieldset className="flex flex-col gap-4">
            ...
            >
              ...
              sklepu i akceptuję jego treść. <span className="text-red-600">*</span>
            </ConsentCheckbox>
            ...
            >
              ...
              i przyjmuję do wiadomości sposób przetwarzania moich danych osobowych zgodnie z RODO.{" "}
              <span className="text-red-600">*</span>
            </ConsentCheckbox>

            <ConsentCheckbox id="marketingOptIn" {...register("marketingOptIn")}>
              Chcę otrzymywać newsletter z ofertami i nowościami (zgoda jest dobrowolna i można ją w
...
function FormField({ label, htmlFor, error, children }: FormFieldProps) {
  return (
    <div className="flex flex-col gap-1.5">
      <label htmlFor={htmlFor} className="text-xs font-semibold uppercase tracking-wide text-zinc-700">
        {label}
      </label>
      {children}
      {error ? (
...
function DeliveryOption({ title, desc, selected, onSelect }: DeliveryOptionProps) {
  return (
    <button
      type="button"
      onClick={onSelect}
...
const ConsentCheckbox = function ConsentCheckbox({
  ...
  return (
    <div className="flex flex-col gap-1">
      <label htmlFor={id} className="flex items-start gap-3 text-sm leading-relaxed text-zinc-700">
        <input
          id={id}
          type="checkbox"
